...
docker run -d --name mc_agent -e "MONGOID_ENV=production" -v /var/run/docker.sock:/var/run/docker.sock --link mc_mongodb:mongodb_1 odaniait/mission-control-agent:latest
docker run -d --name mc_admin -e "RAILS_ENV=production" -e "EXPOSE_PORT=80" -e "PROCESS_TYPE=web" -v /var/run/docker.sock:/var/run/docker.sock --link mc_mongodb:mongodb_1 -p 127.0.0.1:9000:80 odaniait/mission-control:latest
